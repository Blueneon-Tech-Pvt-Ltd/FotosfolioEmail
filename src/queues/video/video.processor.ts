...
export class VideoProcessor {
  ...
  async process(job: Job<VideoProcessJob>): Promise<void> {
    ...
    this.logger.log(
      `Step 3/4: optimizing video for browser playback for ${job.data.key}`,
    );
    ...
    try {
      ...
      try {
        ...
        ({
          ...
          ext,
          contentType,
        } = await this.createFallbackThumbnail(tempDir));
      }
      ...
      if (sourcePath) {
        ...
        try {
          ({
            ...
            ext,
            contentType,
          } = await this.captureThumbnail(sourcePath, tempDir));
        } catch (err: any) {
          if (moovAtEnd) {
      ...
      await this.videoS3Service.uploadBuffer(
        thumbnailKey,
        thumbnailBuffer!,
        contentType!,
      );
      ...
      await rm(tempDir, { recursive: true, force: true }).catch((rmErr) => {
        this.logger.error(
          `Failed to clean up temp directory ${tempDir}: ${this.formatError(rmErr)}`,
        );
      });
    }
  }
  ...
  private async extractMoov(job: Job<VideoProcessJob>): Promise<void> {
    this.logger.log(
      `Moov step: requesting last ${this.MOOV_SCAN_BYTES / 1024 / 1024}MB bytes for ${job.data.key}`,
    );
    const tailBuffer = await this.videoS3Service.getObjectRange(
      job.data.key,
      `bytes=-${this.MOOV_SCAN_BYTES}`,
    );
    const tailMoovBuffer = this.extractMoovBuffer(
      tailBuffer,
      job.data.key,
      'tail',
    );
    ...
    if (!moovBuffer) {
      this.logger.warn(
        `Moov atom not found in scanned ranges for ${job.data.key}; skipping moov extraction`,
      );
      return;
    }
    ...
    this.logger.log(
      `Moov atom extracted for ${moovKey} (${moovBuffer.length} bytes)`,
    );
  }

  private async optimizeVideoForPlayback(
    ...
    try {
      ...
      const keyExt = keyExtMatch ? keyExtMatch[1].toLowerCase() : '.mp4';
      const sourcePath = await this.videoS3Service.downloadRangeToTempFile(
        key,
        undefined,
        tempDir,
        keyExt,
      ...
      this.logger.log(
        `Playback optimization step: remuxing ${key} with faststart`,
      );
      ...
      if (outputStat.size === 0) {
        throw new Error(
          `ffmpeg faststart remux produced empty output for ${key}`,
        );
      }

      const uploadId = await this.videoS3Service.createMultipartUpload(
        key,
        'video/mp4',
      );
      const readStream = createReadStream(outputPath);
      try {
        const completedParts = await this.uploadMultipartStream(
          key,
          uploadId,
          readStream,
        );
        ...
        await this.videoS3Service.completeMultipartUpload(
          key,
          uploadId,
          completedParts,
        );
        ...
        readStream.destroy();
        await this.videoS3Service
          .abortMultipartUpload(key, uploadId)
          .catch((abortErr) => {
            this.logger.error(
              `Failed to abort multipart upload for ${key}: ${this.formatError(abortErr)}`,
            );
          });
        throw err;
      }
      ...
      this.logger.log(
        `Faststart video replaced original object for ${key} (${outputStat.size} bytes)`,
      );
    } catch (err) {
      throw new Error(
        `Failed to optimize video for browser playback for ${key}: ${this.formatError(err)}`,
      );
    } finally {
      await rm(tempDir, { recursive: true, force: true }).catch((rmErr) => {
        this.logger.error(
  ...
  private buildFaststartRemuxArgs(
    sourcePath: string,
    outputPath: string,
  ): string[] {
    return [
  ...
  private async uploadMultipartStream(
    ...
    for await (const chunk of output) {
      ...
      while (pending.length >= this.MULTIPART_PART_SIZE) {
        ...
        completedParts.push(
          await this.videoS3Service.uploadMultipartPart(
            key,
            uploadId,
            partNumber,
            partBuffer,
  ...
  private extractMoovBuffer(
    buffer: Buffer,
    key: string,
    scanPosition: 'head' | 'tail',
  ): Buffer | undefined {
    ...
    if (moovBuffer.length === 0) {
      this.logger.warn(
        `Moov atom is empty in ${scanPosition} range for ${key}; skipping moov extraction`,
      );
      return undefined;
    }
  ...
  private async captureThumbnail(
    ...
    for (const attempt of attempts) {
      for (const seekTime of seekTimes) {
        const outputPath = join(
          tempDir,
          `thumbnail_${seekTime}s${attempt.ext}`,
        );
        this.logger.log(
          `Thumbnail step: writing ffmpeg output to ${outputPath} at ${seekTime}s`,
        );
    ...
    throw new Error(
      `Failed to capture thumbnail after all attempts: ${this.formatError(lastErr)}`,
    );
  }
  ...
  private async createFallbackThumbnail(
    ...
    this.logger.warn(
      `Thumbnail step: using embedded fallback thumbnail in ${tempDir}`,
    );
  ...
  private async supportsWebp(): Promise<boolean> {
    ...
    try {
      const { stdout } = await this.execFileP(ffmpegStatic as string, [
        '-encoders',
      ]);
